...
### Phase 10: מחיקה הדרגתית פשוטה ✅
- [x] חזרה למחיקה פשוטה - step-by-step deletion
- [x] כל חזרה מוחקת את המוצר האחרון
- [x] ביטול התפריט המורכב
- [x] לוגיקה פשוטה: חזור = מחק אחד
- [x] הצגת הודעה על מוצר שנמחק
...
### **התרחישים הנתמכים (גרסה סופית - מחיקה הדרגתית):**

1. **הוספת מוצר רגילה** - בחירה → כמות → מחיר → הוספה לרשימה ✅
2. **עריכה חכמה בחזרה** - חזור אחורה = ערוך את המוצר האחרון ✅
   - מרשימת מוצרים ← חזור = ערוך מחיר של המוצר האחרון
   - ממחיר ← חזור = ערוך כמות של אותו מוצר
   - מכמות ← חזור = ערוך בחירת מוצר
3. **מחיקה הדרגתית** - חזור מרשימה = מחק מוצר אחרון ✅
   - מוצר 1, 2, 3, 4 ← חזור = מחק מוצר 4, נשאר 1,2,3
   - מוצר 1, 2, 3 ← חזור = מחק מוצר 3, נשאר 1,2
   - מוצר 1, 2 ← חזור = מחק מוצר 2, נשאר 1
   - מוצר 1 ← חזור = מחק מוצר 1, חזור לתחילת הזמנה
4. **overwrite או מחיקה** - לפי הקשר ✅
5. **זרימה רציפה** - המשתמש יכול לערוך או למחוק ללא הפרעה ✅

**דוגמה לתרחיש המבוקש:**
- מוצר 1, מוצר 2, מוצר 3, מוצר 4 ✅
- חזור אחורה = מחק מוצר 4, נשאר 1,2,3 ✅
- חזור אחורה = מחק מוצר 3, נשאר 1,2 ✅
- חזור אחורה = מחק מוצר 2, נשאר 1 ✅
- עכשיו אפשר לערוך מוצר 1 או להמשיך ✅
